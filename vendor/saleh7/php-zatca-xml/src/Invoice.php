 ...
 * Updated to place <cac:Signature> in the correct UBL 2.1 position (at the end).
 */
class Invoice implements XmlSerializable
{
    ...
    public function xmlSerialize(Writer $writer): void
    {
        ...
        // InvoiceTypeCode (handled by ZatcaInvoice if overridden)
        if ($this->invoiceType !== null) {
            $writer->write([$this->invoiceType]); // This might be handled differently by ZatcaInvoice
        }
        // Note
        if ($this->note !== null) {
            $writer->write([
        ...
        if ($this->taxTotal !== null) {
            // If taxAmount is separately formatted (check if this logic is needed or redundant)
            // Keeping it as it was, but it might be redundant if $this->taxTotal handles it correctly
                ...
                $writer->write([
                    Schema::CAC . 'TaxTotal' => [
                        Schema::CBC . 'TaxAmount' => [
                            ...
                            'attributes' => [
                                'currencyID' => GeneratorInvoice::$currencyID ?? 'SAR', // Fallback if not set
                            ],
                        ],
                    ],
                ]);
        ...
        // --- CORRECTED POSITION: Signature at the very end ---
        // Signature (Moved to the end to comply with UBL 2.1 schema)
        if ($this->signature !== null) {
            $writer->write([Schema::CAC . 'Signature' => $this->signature]);
        }
        // --- END OF CORRECTED POSITION ---
    }
}
