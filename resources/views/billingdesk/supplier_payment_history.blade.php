...
<body>
    ...
    <div id="content">
        ...
        <h2>Payment History for {{ $suppliers->name ?? 'Supplier' }}</h2>
        @if(session('success'))
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ number_format($transaction->collectedamount, 2) }}</td>
                        <td>
                        ...
                        <td>
                            <a href="{{ url('/payment-voucher/' . $suppliers->id . '/' . $transaction->id) }}" class="voucher-btn" target="_blank">
                                View Voucher
                            </a>
                            
                            @if($branch==2)
                            <form method="POST" action="{{ route('supplierpayment.cancel') }}" style="display: inline;">
                                @csrf
                                <input type="hidden" name="transaction_id" value="{{ $transaction->id }}">
                                <input type="hidden" name="supplier_id" value="{{ $suppliers->id }}">
                                <button type="submit" class="cancel-btn" 
                                    onclick="return confirm('Are you sure you want to cancel this payment of {{ number_format($transaction->collectedamount, 2) }}?')">
                                    Cancel
                                </button>
                            </form>
                            @endif
