        ...
        <div class="row gutters text-center mt-4">
            <div class="col-12">
                ...
                <p class="date-para">
                    ...
                    {{ date('d M Y', strtotime($cash_start_date)) }} To {{ date('d M Y', strtotime($cash_end_date)) }}
                    @endif
                </p>
            </div>
        </div>
            ...
            <div class="col-12">
                ...
                <table class="table table-bordered d-inline-block">
                    <tr>
                        ...
                        <td>{{ $currency }} {{ $updated_balance }}</td>
                    </tr>
                </table>
            </div>
        </div>
                                                ...
                                                <tbody>
                                                    ...
                                                    @foreach ($salesdata as $salesdat)
                                                        <tr>
                                                            <td>{{ $number }}</td>
                                                            <td>
                                                                {{ date('d M Y', strtotime($salesdat->created_at)) }}

                                                                @if ($salesdat->transaction_id != '')
                                                                    <br /><br />
                                                                    Invoice Date:
                                                                    <br />{{ $salesdat->transaction_date }}
                                                                @endif
                                                            ...
                                                            <td>
                                                                @if ($salesdat->comment == 'Payment Received')
                                                                    @if ($salesdat->transaction_id != '')
                                                                        {{ $currency }}
                                                                        {{ $salesdat->collected_amount }} <br /> for
                                                                        payment of
                                                                        {{ $salesdat->transaction_id }}
                                                                    @else
                                                                        {{ $currency }}
                                                                        {{ $salesdat->collected_amount }} paid.
                                                                    @endif
                                                                ...
                                                                @elseif ($salesdat->comment == 'Invoice')
                                                                    @if ($salesdat->transaction_id != '')
                                                                        Transaction: <b>{{ $salesdat->transaction_id }}
                                                                        </b><br />
                                                                    @endif
                                                                @elseif ($salesdat->comment == 'Product Returned')
                                                                    @if ($salesdat->transaction_id != '')
                                                                        {{ $currency }}
                                                                        {{ $salesdat->collected_amount }} <br />
                                                                        returned on
                                                                        transaction
                                                                        {{ $salesdat->transaction_id }} <br /> <br />
                                                                    @else
                                                                        {{ $currency }}
                                                                        {{ $salesdat->collected_amount }} returned.
                                                                        <br /> <br />
                                                                    @endif
                                                                @elseif ($salesdat->comment == '')
                                                                    @if ($salesdat->transaction_id != '')
                                                                        Transaction {{ $salesdat->transaction_id }}
                                                                        <br />
                                                                    @endif
                                                            ...
                                                            <td>
                                                                @if ($salesdat->collected_amount != null || $salesdat->collected_amount != 0)
                                                                    <b>{{ $currency }}</b>
                                                                    {{ $salesdat->collected_amount }}
                                                                @endif
                                                            </td>
                                                            <!-- {{-- <td><b>{{ $currency }}</b>
                                                                {{ $salesdat->updated_balance }}</td> --}} -->
                                                        </tr>
                                                        <?php $number++; ?>
