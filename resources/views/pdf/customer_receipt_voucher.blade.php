...
<body>
    ...
    <table class="voucher-details">
        ...
        <tr>
            <th>Customer TRN:</th>
            <td>{{ $customer->trn_number ?? 'N/A' }}</td>
            <th>Payment Method:</th>
            <td><strong>{{ $paymentType }}</strong></td>
        </tr>
        <tr>
            <th>Customer Address:</th>
            <td colspan="3">{{ $customer->billing_add }}</td>
        </tr>
    </table>
    ...
    <table class="voucher-details">
        ...
        <tr>
            ...
            <td class="text-right amount-cell">{{ number_format($transaction->collected_amount, 2) }}</td>
        </tr>
        <tr>
            <th class="text-right">Total Amount:</th>
            <th class="text-right amount-cell">{{ number_format($transaction->collected_amount, 2) }}</th>
        </tr>
    </table>
    <div class="balance-info">
        <strong>Customer Account Balance:</strong> 
        {{ number_format($balance, 2) }} {{$currency}}
    </div>

    <div class="footer">
        <div>Prepared by: ___________________</div>
        <div>Customer Acknowledgement: ___________________</div>
    </div>
