  ...
  <div style="flex: 1; margin: 0 3px;">
    <a href="/salesreport" style="display: block; padding: 10px 5px; background-color: #187f6a; color: white; text-align: center; text-decoration: none; border: 1px solid #146354; border-radius: 4px; transition: all 0.3s ease;">Sales Report</a>
        ...
        <div class="input-group" style="margin-bottom: 10px;">
            <a href="/export-salesreturn-monthsdata/{{ $userid }}/{{ $viewtype }}/{{ $location }}/{{ $fromdate }}/{{ $todate }}"
        ...
        <table class="table">
            ...
            @endphp
                    ...
                    <td><b>{{ $currency }}</b> {{ number_format($grand_total_with_discount, 3) }}</td>
                        ...
                        <td>
                            <a href="/monthreturn/{{ $productday->month_id }}/{{ $location }}"
                                class="btn btn-primary">VIEW</a>

                            <div class="input-group export">
                                <a href="/export-salesreturn-month/{{ $userid }}/{{ $viewtype }}/{{ $location }}/{{ $productday->date }}"
                                    class="btn btn-primary">Export</a>
                            </div>
                        </td>
                @endforeach
