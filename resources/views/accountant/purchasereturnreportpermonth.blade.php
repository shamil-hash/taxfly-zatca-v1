...
<body>
  ...
  <div style="flex: 1; margin: 0 3px;">
    <a href="" style="display: block; padding: 10px 5px; background-color: #115c4c; color: white; text-align: center; text-decoration: none; border: 1px solid #0d4539; border-radius: 4px; font-weight: bold; box-shadow: inset 0 2px 4px rgba(0,0,0,0.15);" >Purchase Return Report</a>
  </div>
  <div style="flex: 1; margin: 0 3px;">
  ...
  <div style="flex: 1; margin: 0 3px;">
    ...
    <h2 ALIGN="CENTER">Purchase Return Report</h2>
    <div class="input-group" style="margin-bottom: 10px;">
      <a href="/export-purchasereturn-monthsdata/{{$userid}}/{{$viewtype}}/{{$location}}/{{$fromdate}}/{{$todate}}" class="btn btn-primary">Export</a>
    </div>
    <table>
      <tr>
        <th>Date</th>
      ...
      <tr>
        <td>{{ $purchase->date }}</td>
                    ...
                    <td>
                        ...
                        <!--@endif-->
                        {{ $purchase->amount - $purchase->price_without_vat }}
                    </td>
                    <td>
                        <b>{{ $currency }}</b>
                    ...
                    <td><b>{{ $currency }}</b> {{ $purchase->amount -$purchase->discount }}</td>
        <td>
          <a href="/monthpurchasesreturn/{{$purchase->month_id}}/{{$location}}" class="btn btn-primary">VIEW</a>

          <div class="input-group export">
            <a href="/export-purchasereturn-month/{{$userid}}/{{$viewtype}}/{{$location}}/{{$purchase->date}}" class="btn btn-primary">Export</a>
          </div>
        </td>
      </tr>
      @endforeach
